...
* [outputs/provenance_mpi](outputs/provenance_mpi): the two Turtle files
  ...
  They were used to generate visualization graphs (*.gexf) in this folder.
* [outputs/provenance_snakemake](outputs/provenance_snakemake): the folder
  `psds_by_trial_type` contains the intermediate pickle files generated by each
  run of the `compute_psd_by_trial_type.py` script. For each run, a Turtle file
  with the session name is saved (`i140703-001.ttl` and `l101210-001.ttl`) with
  the provenance information, corresponding to the NIX file processed. An
  ...
  files are used to generate the graph presented in Figure 14B (specific
  details below).
* [outputs/smoothed_plot](outputs/smoothed_plot): the outputs are presented as 
  ...
  visualization graph presented in Figure 13B (specific details below).  
...
* Figure 14B: [outputs/provenance_snakemake/R2G_PSD_all_subjects_simplified.gexf](outputs/provenance_snakemake/R2G_PSD_all_subjects_simplified.gexf)
